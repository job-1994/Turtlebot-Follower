...
Follow::Follow(ros::NodeHandle nh_, image_transport::ImageTransport it_)
{
    image_sub_ = it_.subscribe("/camera/rgb/image_raw", 1, &Follow::imageCb, this, image_transport::TransportHints("compressed"));
...
float Follow::divisor(float number, int power)
{
...
void Follow::imageCb(const sensor_msgs::ImageConstPtr& msg)
{
...
//Trackbar "Colour" links to the Colour variable, and the following settings link to the corresponding int values
